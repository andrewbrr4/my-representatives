...
> 1. ~~**Streaming the formatter output**~~ — **shipped 2026-05-30** (NDJSON `model.astream` + `update_partial`, default on; per-node progress bar + facts carousel for the loading phase). Perceived-latency win realized: first bullets land well before the full block. Remaining tuning: calibrate `PROGRESS_STEPS` percentages against observed node timings.
...
- [x] **[L]** Stream the bullets so the user sees the first 1–2 within a few seconds rather than waiting 26s for the full block — _shipped 2026-05-30: the formatter now emits NDJSON (one `{"text","sources"}` object per line) via `model.astream` instead of a blocking `with_structured_output` call (`OVERVIEW_V4_FORMATTER_STREAMING`, default on; structured path retained as fallback). Each parsed line is written to the research store via `update_partial`, so the 2s frontend poll renders bullets as they land — first bullet now appears well before the full block completes. The streaming path uses its own prompts (`formatter_system_streaming.txt` / `formatter_user_streaming.txt`) since the structured prompt prescribes the `bullet_texts`/`bullet_sources` tool schema; citation/`[N]`-marker assembly and hallucinated-URL dropping happen in Python (`_handle_line`) exactly as in the structured path. Paired with this: each pipeline node reports a `(label, pct)` step (`research/overview/progress.py` → `update_progress` → `ResearchResponse.progress`) driving a per-node loading progress bar, plus a DB-served fun-facts carousel (`facts` table → `GET /api/facts` → `useFactsQuery`) shown until the first bullet retires the progress view. **Follow-up:** calibrate `PROGRESS_STEPS` percentages against observed per-node timings (current values are a first draft)._
