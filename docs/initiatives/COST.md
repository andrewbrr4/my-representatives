...
Two tables in Cloud SQL PostgreSQL. Migration: `backend/migrations/001_create_research_tasks_and_transactions.sql`.

```
research_tasks
  - id              text PK (12-char hex research_id)
  - representative  text (e.g. "Jane Doe (U.S. Senator, New York)")
  - input_tokens    int
  - output_tokens   int
  - tool_calls      int
...
transactions
  ...
  - description     text
  - research_task_id text FK → research_tasks(id), nullable
  - created_at      timestamptz
  - balance_after   numeric(10,4) (running total)
```

`research_tasks` captures per-rep research telemetry (one row per on-demand research request). `transactions` is the financial ledger. `research_task_id` FK links per-request outflows to specific research tasks; null for bulk/subscription/inflow entries. `source` is freeform so new cost sources can be added without a schema migration.
...
Usage bubbles up from section → rep:
...
- The research router logs per-task usage and persists to the database

**Database persistence:** After research completes, usage data is written to the `research_tasks` table in Cloud SQL PostgreSQL via `db.py` (asyncpg connection pool). DB writes are wrapped in try/except so failures never break the main research flow.
...
Anthropic and Tavily outflow transactions are written to the `transactions` table inline at request time, alongside `save_research_task`. Cost is computed from token counts and tool calls using env var pricing (`ANTHROPIC_INPUT_COST_PER_M`, `ANTHROPIC_OUTPUT_COST_PER_M`, `COST_PER_SEARCH`). See `db.py:save_transactions()`.

**Design decision: two tables, two purposes.**

- **`research_tasks`** = operational telemetry. Raw token counts, tool calls, model, and pricing snapshot per research request. Used by the dashboard for per-request stats (average cost per research, tokens per request). Pricing is captured at write time so historical data survives rate changes.
...
**Dashboard (`/costs`):**
- Per-request stats from `research_tasks`: average cost per research, tokens/searches per request
