...
export function useIssueSearch(rep: Representative) {
  ...
  const getEntry = useCallback(
    (key: string): IssueEntry => {
      return queryClient.getQueryData<IssueEntry>(["rep-issue-research", key]) ?? {
        status: "idle",
        summary: null,
        researchId: null,
        issue: null,
  ...
  const setEntry = useCallback(
    (key: string, entry: IssueEntry) => {
      queryClient.setQueryData(["rep-issue-research", key], entry);
    },
    [queryClient]
  );
  ...
  const cacheVersion = useQuery({
    queryKey: ["rep-issue-research-version", rep.name, rep.office],
    queryFn: () => 0,
    initialData: 0,
    staleTime: Infinity,
  });
  ...
  const bumpVersion = useCallback(() => {
    queryClient.setQueryData<number>(
      ["rep-issue-research-version", rep.name, rep.office],
      (v) => (v ?? 0) + 1
    );
  ...
  useEffect(() => {
    ...
    for (const query of cache) {
      ...
      if (qk[0] === "rep-issue-research" && qk.length === 2 && typeof qk[1] === "string") {
