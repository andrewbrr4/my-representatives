...
- **v4** (`research/overview/v4/`) — LangGraph-native breadth + adaptive depth. A top-level `StateGraph(V4State)` wires `query_generator → breadth_search → filter → research_agent → formatter`. The research_agent is a `create_react_agent` (LangGraph prebuilt) with one tool, `request_depth_research`. That tool invokes a depth subagent that is itself a `create_react_agent` over an isolated `DepthState`; the subagent's only tool, `depth_tavily_search`, returns `Command(update={search_results: [SearchResult], messages: [ToolMessage]})` so structured search results accumulate in `DepthState.search_results` while formatted snippets continue to drive the agent loop via `messages`. State isolation across subagent boundaries prevents the token-accumulation problem v1/v2 suffered: a depth subagent's `messages` (Tavily ToolMessage snippets, agent reasoning) lives and dies in `DepthState` — only `SearchResult` lists cross back. `request_depth_research` tags depth results with the topic and pushes them up as `depth_search_results` to research_agent, which forwards them unchanged to V4State. The formatter takes `filtered_results` + `depth_search_results` (both `list[SearchResult]`, fully symmetric) and does curation+presentation in one structured-output call. The output schema is two **parallel top-level lists** indexed in lockstep — `bullet_texts: list[str]` and `bullet_sources: list[list[str]]` — never a nested `list[Bullet]`. The flat shape is what v2/v3 use reliably; the original v4 nested-list schema caused Sonnet 4.6 to stringify `bullets` as a JSON-encoded string in ~40% of runs (silent Pydantic validation failures, no user-visible output). The formatter wraps the structured-output call in LangChain's `with_retry(retry_if_exception_type=(ValidationError,), stop_after_attempt=2)` so any residual wire-shape miss (e.g. one of the parallel lists arriving as a stringified JSON array) gets one retry — empirically, the second attempt usually emits the correct shape. The user prompt ends with an explicit primacy/recency reminder of the wire shape (`bullet_texts` and `bullet_sources` must each be JSON arrays, not stringified). Citations are assembled in Python from `bullet_sources` (URL first-appearance order, deduped, looked up against the combined breadth+depth pool) and `[N]` markers are appended to bullet text in Python — the LLM never emits markers, so there's no chance of N mismatch. `TOTAL_SECTIONS=1`. Prompts in `research/overview/v4/prompts/`. Tunable via `OVERVIEW_V4_*` env vars (see below).
