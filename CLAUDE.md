...
**Rep overview pipeline** lives in `research/overview/` and is versioned. The active version is selected at import time via the `OVERVIEW_PIPELINE_VERSION` env var. **Production runs `v4`** (LangGraph breadth + adaptive depth + structured-output formatter). `v1` is also actively supported as the alternative user-facing paradigm (per-section streaming with skeletons — different UX from v4's single-block render). `v2` and `v3` are kept available for trace/cost comparison but are subsumed by v4 and not the focus of further iteration. Each version package exports `ResearchSummary`, `research_representative`, and `TOTAL_SECTIONS`. **Each version owns its own `ResearchSummary` Pydantic model** (`v{N}/models.py`) — there is no shared overview-model module. Bullet-shaped summaries (v2, v3, v4) all define `ResearchSummary(bullets: list[str], citations: list[Citation])` with `bullets` as a required, non-nullable list (empty list = loading state). The previously-shared `list[str] | None` generated an `anyOf[array, null]` JSON schema that occasionally caused Anthropic to emit `bullets` as a JSON-encoded string — removing the null removed the ambiguity. All variants use LangChain + Langfuse tracing with version-prefixed `@observe` names (e.g. `v1-research-pipeline`, `v2-synthesis`, `v3-distill`, `v4-formatter`) and a `UsageTracker` callback (`research/usage.py`) for token/tool-call accounting. See `docs/rep-overview-versions.md` for the rationale behind each version.
...
- `OVERVIEW_PIPELINE_VERSION` — which rep overview pipeline to run. **Production: `v4`** (LangGraph breadth + adaptive depth). Alternative actively-supported paradigm: `v1` (5 section agents, streaming UX). Legacy / kept for comparison: `v2` (sections → synthesis bullets), `v3` (static-query fan-out → distill bullets). Code default is `v1` as a fallback if the env var is unset; prod sets `v4` explicitly on Cloud Run. Read at import time by `research/overview/__init__.py`; also encoded into `research_tasks.task_type` (`rep:v1`/`rep:v2`/`rep:v3`/`rep:v4`) and into Langfuse trace names.
