...
9. `db.py` manages an `asyncpg` connection pool (lazy singleton) for Cloud SQL PostgreSQL. Contains `save_job()` for persisting per-request usage data. The pool is created on first use and closed on app shutdown. SQL migrations live in `migrations/`.
